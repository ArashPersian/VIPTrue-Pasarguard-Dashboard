...
requireText('upstream theme customization retained', themeProvider, 'applyThemeCustomization(composeTheme(')
requireText('VIPTrue surface integration', themeProvider, 'getVIPTrueThemeSurface(themeVars, themeMode)')

const colorThemes = await read('dashboard/src/constants/color-themes.ts')
requireText('VIPTrue default theme', colorThemes, "default: basePalette('VIPTrue', 342, 8, 6, '342 100% 57%'")
requireText('VIPTrue default theme swatch', colorThemes, "default: '#ff3d8d'")
requireText('theme sidebar primary retained', colorThemes, "'--sidebar-primary': lightPrimary")

const themeSurfaces = await read('dashboard/src/brand/theme-surfaces.ts')
requireText('theme surface primary derivation', themeSurfaces, "parseHsl(themeVars['--primary']")
requireText('theme surface neon token', themeSurfaces, "'--viptrue-neon'")
...
const themePreview = await read('dashboard/src/features/theme/theme-preview.tsx')
requireText('theme-aware live preview', themePreview, 'data-testid="viptrue-theme-preview"')
requireText('live preview logo', themePreview, '<BrandLogo compact')
...
requireText('WireGuard node route', sidebar, "url: '/nodes/wireguard'")
